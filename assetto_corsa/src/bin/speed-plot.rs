...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut theorectical_gear_data_points: Vec<Vec<(f64, f64)>> = Vec::new();
    let mut current_real_gear_vec: Vec<(f64, f64)> = Vec::new();
    let mut current_theorectical_gear_vec: Vec<(f64, f64)> = Vec::new();
    ...
    loop {
        if engine_rpm > rpm_limit {
            ...
            theorectical_gear_data_points.push(current_theorectical_gear_vec);
            current_real_gear_vec = Vec::new();
            current_theorectical_gear_vec = Vec::new();
        }
        theoretical_speed = car.engine_rpm_to_wheel_speed(engine_rpm, current_gear_idx);
        current_theorectical_gear_vec.push((theoretical_speed * 3.6, engine_rpm));
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption("Ratios Graph", ("Arial", 20).into_font())
        .set_label_area_size(LabelAreaPosition::Bottom, 40)
        .set_label_area_size(LabelAreaPosition::Left, 40)
        .build_cartesian_2d(0f64..((max_speed * 3.6)+10f64), 0f64.max(car.min_rpm_needed_to_move_from_stationary()-500f64)..rpm_limit+1000f64)?;
    ...
    for (idx, data) in theorectical_gear_data_points.into_iter().enumerate() {
        chart
